...
#        kwargs = {'num_workers': args.workers, 'pin_memory': True}
        ...
        # Define network
            model = DeepLab(backbone=args.backbone,
                            output_stride=args.out_stride,num_classes=self.nclass,
                            sync_bn=args.sync_bn,
                            freeze_bn=args.freeze_bn, pretrained=True)
        ...
        if args.ft:
            args.start_epoch = 0
    def training_truncatedloss(self, epoch):
        train_loss = 0.0
        self.model.train()
        for batch_idx, (inputs, targets, indexes) in enumerate(self.train_loader):
            inputs, targets = inputs.cuda(), targets.cuda()
            self.scheduler(self.optimizer, batch_idx, epoch, self.best_pred)
            self.optimizer.zero_grad()
            
            outputs = self.model(inputs)
            loss = self.criterion(outputs, targets, indexes)
...
#            image, target = sample[0], sample[1]
        ...
        self.writer.add_scalar('val/precision', IoU, epoch)

        print('Validation:')
        print('[Epoch: %d, numImages: %5d]' % (epoch, i * self.args.batch_size + image.data.shape[0]))
        print("Acc:{}, Acc_class:{}, IoU:{}".format(Acc, Acc_class, IoU))
        ...
        if epoch == 0:
            ...
            log_file.write(str(self.nclass) + 'classes \n')
        log_file.write('Epoch: %d, ' % (epoch + 1))
        if new_pred < self.best_pred:
            log_file.write('Acc: {}, Acc_class: {}, IoU: {}, best_mIoU: {}, '.format(Acc, Acc_class,  IoU, self.best_pred))
            # log_file.write('Acc: {}, Acc_class: {}, mIoU: {}, fwIoU: {}, best_mIoU: {}, '.format(Acc, Acc_class, mIoU, FWIoU, self.best_pred))
        else:
            log_file.write('Acc: {}, Acc_class: {}, IoU: {}, best_mIoU: {}, '.format(Acc, Acc_class, IoU, new_pred))
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='inria',
                        choices=['pascal', 'coco', 'cityscapes', 'potsdam','lulc','inria','inria_chicago_optimize'],
    ...
    # # # 使用预训练模型
    # parser.add_argument('--resume', type=str, default='/home/zjh/phd_annotate/run/inria/unet/model_best.pth.tar',
    #                     help='put the path to resuming file if needed')  # 恢复文件
