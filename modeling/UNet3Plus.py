# -*- coding: utf-8 -*-
"""
Created on Wed Dec 23 15:54:40 2020
...
from modeling.layers import unetConv2
from modeling.init_weights import init_weights
import gdal,os
...
def featuremap(input):
    ...
    input = input.cuda()
    # print(input.shape)
    model = UNet3Plus(n_channels=3,n_classes=6)
    model =model.cuda()
    model.eval()
    output = model(input)
...
def main():
    ...
    #         tile_pred = predict.cpu().numpy()
            ...
            dst_nbands = 1
            # dst_nbands = 6
            file_dst=os.path.join(output_path, rastername.split('.tif')[0]+'_%s_unet3+.tif'%i)
            print(file_dst)
