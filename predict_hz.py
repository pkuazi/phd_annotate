...
from utils.gen_tiles_offs import gen_file_list,gen_tiles_offs
...
def main():
    ...
    BLOCK_SIZE=512
    OVERLAP_SIZE=0
    files_list = gen_file_list(input_path,BLOCK_SIZE,OVERLAP_SIZE)
    
    # model = '/home/zjh/phd_annotate/run/inria/unet/70epoch_model_best.pth.tar'
    model = '/home/zjh/phd_annotate/run/inria_chicago_optimize/unet/random_55epoch_model_best.pth.tar'
    ...
    for i in range(num):
        ...
        print(xoff, yoff)
        img_tif = ds.ReadAsArray(xoff,yoff,BLOCK_SIZE,BLOCK_SIZE)
        img_tif = img_tif[:3]
...
#         tile_pred = predict.cpu().numpy()
        pred_arr[yoff:yoff+BLOCK_SIZE,xoff:xoff+BLOCK_SIZE]=pred_
    
    # dst_file = '/tmp/clt2.tif'
    dst_file = '/home/zjh/tmp/gf1c_area.tif'

    dst_format = 'GTiff'
    driver = gdal.GetDriverByName(dst_format)
    dst_ds = driver.Create(dst_file, xsize, ysize, 1, 1)
...
if __name__ == '__main__':
    ...
    # #     # print(pred.max())
    # #     print('[%s] Finished test.' % datetime.now())
    #     # array_to_tiff(pred_, out_proj, out_gt, 1, os.path.join(result_path,'pred178_%s_%s_%s.tif'%(filename.split('.')[0],xoff, yoff)))
    ...
    #     label = os.path.join(label_path,filename)
    #     ds_l=gdal.Open(label)
    #     y_true=ds_l.ReadAsArray(xoff, yoff, tile_size, tile_size)
        
    #     tmp = np.zeros(list(y_true.shape))
    #     tmp[y_true==255]=1
    #     y_true=tmp
    #     # cv2.imwrite(os.path.join(result_path,'label_%s_%s_%s.tif'%(filename.split('.')[0],xoff, yoff)),y_true)
        
    #     # iou_score = jaccard_similarity_score(pred_, y_true, Normalize = True/False)
    #     intersection = np.logical_and(pred_, y_true)
    #     union = np.logical_or(pred_, y_true)
    #     if np.sum(union)==0:
    #         continue
    #     iou_score = np.sum(intersection) / np.sum(union)
    #     acc_score = np.sum(pred_==y_true)/(512*512)
    #     print('IoU is %s' % iou_score)
    #     i=i+1
    #     fasle_pred = y_true==pred_
    #     fasle_pred=fasle_pred.astype('int8')
    #     # array_to_tiff(fasle_pred, out_proj, out_gt, 1, os.path.join(result_path,'falsepred178_%s_%s_%s.tif'%(filename.split('.')[0],xoff, yoff)))
    ...
    # print('final iou,',score/i)
    # print('final accuracy',a_score/i)
