...
#     feat.append(var(bd[bd!=0]))
    ...
    return [ avg, std]
def mask_feats_by_geometry(poly_coords,poly_proj, raster):
    import time
    T1 = time.perf_counter()
    
    # transform = GeomTrans(str('EPSG:4326'), str(raster.crs.wkt))
    transform = GeomTrans(poly_proj, str(raster.crs.wkt))
...
if __name__ == '__main__':
    ...
    # feats1 = main()
    # build overviews
    data1 = '/mnt/rsimages/ISPRS_BENCHMARK_DATASETS/Potsdam/11_deeplab_feature/top_potsdam_2_10_RGB_deeplabv3+.tif'
    # cmd = 'gdaladdo /mnt/rsimages/work/phd_test/top_potsdam_2_10_RGB_deeplabv3+.tif'
    cmd = 'gdaladdo /mnt/rsimages/work/phd_test/n42_20_2000lc030.tif'
    T1 = time.perf_counter()
    # raster = rasterio.open(rasterfile, 'r')
    # dboxds=dboxio.Open(rasterfile)
    os.system(cmd)
    ...
    # cmd2 = 'dboxgdaladdo /mnt/rsimages/work/phd_test/dbox/top_potsdam_2_10_RGB_deeplabv3+.DBOX'
    cmd2 = 'dboxdataset addo /mnt/rsimages/work/phd_test/n42_20_2000lc030.tif'
    T1 = time.perf_counter()
    # raster = rasterio.open(rasterfile, 'r')
    # dboxds=dboxio.Open(rasterfile)
    os.system(cmd2)
